...
//构造函数
    var Validator = function (formEle, rules) {
...
//类变量、函数（静态变量、静态函数）
    ...
    "^\\d{6}$",						//邮编
        mobile
    :
    "^(13[0-9]{9}|15[012356789][0-9]{8}|18[0256789][0-9]{8}|147[0-9]{8}$)",				//手机
        ip4
    :
...
:
    "qq格式输入不正确",
        tel
:
    "电话号码格式错误",	//电话号码的函数(包括验证国内区号,国际区号,分机号)
        username
:
    "用户名只能包含英文、数据、下划线",						//用来用户注册。匹配由数字、26个英文字母或者下划线组成的字符串
        letter
:
    "字母",					//字母
        uppercase
...
,
    ...
    attrRules: function (element, attr) {
        ...
        try {
            ...
            Validator.debug("valid rule error:" + data);
        }
